"""
More Informations about the Bullet Character Controler can be found at:
http://www.panda3d.org/forums/viewtopic.php?t=13904
source code:
...
class PandaBulletCharacterController(object):
    ...
    def __processFlying(self):
        if self.__footContact and \
               self.__currentPos.z - 0.1 < self.__footContact[0].z and \
               self.__linearVelocity.z < 0.0:
            self.__currentPos.z = self.__footContact[0].z
            self.__linearVelocity.z = 0.0

        if self.__headContact and \
               self.__capsuleTop >= self.__headContact[0].z and \
               self.__linearVelocity.z > 0.0:
            self.__linearVelocity.z = 0.0
